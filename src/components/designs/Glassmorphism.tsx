    ...
    <div className="min-h-screen relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0f0c29 0%, #1a1145 50%, #16132e 100%)", color: "#e8e8f0" }}>
      ...
      <div className="relative z-10">
        ...
        <div className="px-8 pt-10 pb-6 max-w-[1400px] mx-auto">
          <h1 className="text-3xl font-light tracking-tight" style={{ fontFamily: "'Inter', sans-serif" }}>
            Strategic Intelligence
          </h1>
          <p className="text-sm mt-1 opacity-50">MyCFO Autonomous Oversight</p>
            ...
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                ...
                <div>
                  ...
                  <div className="text-[10px] uppercase tracking-wider opacity-50">{s.label}</div>
                </div>
              </motion.div>
            ))}
          </div>
            ...
            <div className="space-y-3">
              {priceDriftItems.map((item) => (
                <div key={item.id} className="flex items-center gap-3">
                  <span className="text-xs w-28 truncate">{item.product}</span>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5 }} className={glass}>
            ...
            {arbitrageOpportunities.map((opp) => (
              <div key={opp.id} className="mb-3 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.05)" }}>
                <div className="text-xs font-medium mb-2">{opp.product}</div>
